...
public class Decoder extends UnicastRemoteObject implements DecodingInterface
{
    ...
    public String decodeVigenere(String key) throws RemoteException
    {
        ...
        char[][] tabular = new char[26][26];
        try
        {
            BufferedReader bufferedReader = new BufferedReader(new FileReader(new File("TabulaRecta.txt")));
            String line;
            int count = 0;
            while((line = bufferedReader.readLine()) != null)
            {
                tabular[count] = line.toCharArray();
                count++;
            }
        }
        ...
        for(int i=0;i<cipher.length(); i++)
        {
            ...
            {
                int rotNo = (int)key.charAt(i % key.length()) - 65;
                int columnNo = (int)cipher.charAt(i) - 65;
                //ch = (char)(((((int)cipher.charAt(i) + (int)key.charAt(i%key.length())) + 26) % 26) + 65);
                ch = tabular[rotNo][columnNo];
